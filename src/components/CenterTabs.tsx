...
export default function CenterTabs({ locale, slug, active }: Props) {
  ...
  return (
    <div className="md:w-[920px] w-full sm:my-10 my-5 grid grid-cols-3 sm:grid-cols-3 justify-center items-stretch bg-lightBorder text-secondary rounded-3xl overflow-hidden">
      {tabs.map((tab) =>
        tab.id === active ? (
          <p
            key={tab.id}
