...
// Interface for the detailed event page
interface EventDetail {
  ...
  schedule: Schedule;
  speakers: Speaker[];
  categories: EventCategory[]; // Corrected from event_category_event
}

// Interface for the list of events ("You may also like")
interface EventItem {
  id: number;
  slug: string;
  title: string;
...
const Page = () => {
  ...
  // Fetch other events for the "You may also like" section
  ...
  const similarEvents =
    otherEventsData?.data.filter((event) => event.slug !== slug).slice(0, 3) ||
    [];
            ...
            <>
              <div className="w-full md:h-[470px] h-[220px] relative">
                <Swiper
                  ...
                  loop={eventData.galleries.length > 1}
                ...
                {/* FIX: Changed to use the 'categories' array */}
                ...
                <div className="bg-backgroundSecondary border border-lightBorder p-5 flex_start flex-col gap-5 md:rounded-3xl rounded-xl lg:w-auto w-full lg:min-w-[400px]">
                  ...
                  <div className="flex justify-between items-center md:flex-nowrap flex-wrap gap-3 w-full px-3 py-2 md:rounded-3xl rounded-xl border border-lightBorder bg-white">
                    <div className="flex_center gap-3">
                      ...
                      <p className="text-sm text-black text-opacity-60">
                        {eventData.schedule.date_string}
                      </p>
                    </div>
                    <div className="flex_center gap-3">
                      <span className="bg-secondary bg-opacity-30 rounded-full flex_center w-7 h-7">
                      ...
                      <p className="text-sm text-black text-opacity-60">
                        {eventData.schedule.time_string}
                      </p>
                    </div>
                    <div className="flex_center gap-3">
                      <span className="bg-secondary bg-opacity-30 rounded-full flex_center w-7 h-7">
                  ...
                  <div className="flex justify-between items-center gap-3 w-full px-3 py-2 md:rounded-3xl rounded-xl border border-lightBorder bg-white">
                    <div className="flex_center gap-3">
                      ...
                      <p className="text-sm text-black text-opacity-60">
                        {eventData.schedule.payment_type}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 w-full gap-8">
                    {similarEvents.map((event) => (
                      <EventCard
                        key={event.id}
                        image={
                          event.galleries[0]?.image.lg || "/images/event.png"
                        }
                        link={`/${locale}/events/${event.slug}`}
                        // FIX: Changed to use the 'categories' array for the event card as well
